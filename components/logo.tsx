...
const Logo = ({ smallLogo }: props) => {
    ...
    return (
        <div className={clsx(`flex items-center justify-center`, smallLogo ? ("py-2 pr-1 flex-col") : ("mb-3"))}>
            ...
            <p className={clsx(`ml-1 font-bold font-sans`, smallLogo ? ("text-[0.7rem] text-[#242424]") : ("text-gray-700 text-[1.2rem]"))}>Chat<span className={clsx(`font-extrabold`, smallLogo ? ("text-[#eee]") : ("text-violet-500"))}>zz</span></p>
        </div>
    )
}
